...
class FiniteDifferences(OptionsPricing):
    ...
    def __init__(self, S0, K, r, T_start, T_end, sigma, Smax, M, N, u0, is_call=True):
        super(FiniteDifferences, self).__init__(S0, K, r, T_start, T_end, sigma, u0, is_call)
        self.Smax = Smax
        self.M, self.N = int(M), int(N)  # Ensure M&N are integers
        self.dS = Smax / float(self.M)
        self.dt = (T_end - T_start) / float(self.N)
